...
def read_examples(input_file, is_training):
    ...
    df['content'] = df['content'].fillna('无')
    df['entity'] = df['entity'].fillna('无')
    examples=[]
    for val in df[['id','content','entity','label']].values:
...
def accuracy(out, labels):
    ...
    return accuracy_score(labels, outputs, normalize=True)
...
def main():
    ...
    parser.add_argument('--device_id', type=str, default='0', help="The CUDA device is for training.")
    ...
    if args.do_test:
        ...
        for file,flag in [('dev.csv','dev'),('test.csv','test')]:
            ...
            if flag=='test':
                ...
                df['label_1']=logits[:,1]
                #df['label_2']=logits[:,2]
                df[['id','entity','label_0','label_1']].to_csv(os.path.join(args.output_dir, "test_pb.csv"),index=False)
